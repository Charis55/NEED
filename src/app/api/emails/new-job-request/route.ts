...
export async function POST(req: Request) {
  try {
    const { artisanId, customerName, trade, neighborhood, preferredTime } = await req.json();
    ...
    const data = await resend.emails.send({
      ...
      to: [email],
      subject: 'New Job Request on NEED!',
      react: NewJobRequestEmail({ technicianName, customerName, trade, neighborhood, preferredTime }),
    });

    return NextResponse.json({ success: true, data });
  } catch (error: any) {
    console.error('Error sending new job request email:', error);
